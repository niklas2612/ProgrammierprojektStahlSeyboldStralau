...
#define ll_new(ll) (decltype (ll)) _ll_new(ll, sizeof(*ll))
#define ll_pop(ll) (decltype (ll)) _ll_pop(ll)
#define ll_next(ll) (decltype (ll)) _ll_next(ll)
#define ll_reduce(ll, fn, val) (decltype (ll)) _ll_reduce(ll, fn, val)

#define ll_foreach(ll, item) \
    for (decltype (ll) item = ll; item; item = _ll_next(item))
